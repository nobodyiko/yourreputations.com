...
class Lazy_Images {
	...
	public static function get_placeholder_image($id, $size = 'full'){
		$meta_key = 'lazy-placeholder-'.(is_array($size) ? join('x', $size) : (string)$size);
		$placeholder = get_post_meta($id,$meta_key, true);
		if ('' === $placeholder) {
			$placeholder = static::transparentImage($id, $size);

			update_post_meta($id,$meta_key, $placeholder);
		}
	...
	protected static function transparentImage($id, $size) {
		...
		$w = max(1, round($width/10));
		$image = imagecreatetruecolor(intval($w), intval(max(1, round($w/(round($width/$height, 2)), 2))));
