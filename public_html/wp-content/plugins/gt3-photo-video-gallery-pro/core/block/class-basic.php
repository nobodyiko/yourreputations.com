...
//					'script'          => 'gt3pg_pro--core',
	...
	protected function serializeImages(&$settings){
		...
		$settings['ids'] = array_filter($settings['ids'], function($value){
			if (!is_array($value) || !((key_exists('width', $value) && (int)$value['width']>0) || (key_exists('height', $value) && (int)$value['height'])>0)) {
				return false;
			}
