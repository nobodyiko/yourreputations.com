...
if ( !post_password_required() ) {
	...
	$layout = apply_filters('gt3_archive_pf_layout', $layout);
		...
		<div class="row<?php echo esc_attr($row_class); ?>">
			<div class="content-container span<?php echo (int)esc_attr($column); ?>">
				<section id='main_content'>
					...
					$category_archive_layout = gt3_option('portfolio_archive_layout');
					if (is_null($category_archive_layout)) {
						$category_archive_layout = 3;
					}

				        $port -> set_settings(apply_filters( 'gt3/core/cpt/portfolio/archive_portfolio/set_settings', array(
				        		'cols' => apply_filters('gt3_archive_pf_columns11', $category_archive_layout),
				        		'show_category' => 'yes',
