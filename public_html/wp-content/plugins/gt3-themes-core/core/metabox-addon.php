...
function gt3_get_all_icon () {
	...
	$file = stream_resolve_include_path(get_template_directory() . '/css/font-awesome.min.css');
	if (false === $file) {
		$file = stream_resolve_include_path(get_template_directory() . '/dist/css/font-awesome.css');
	}
	if (false === $file) {
		return array();
	}
...
if ( class_exists( 'RWMB_Field' ) )
{
	if (is_plugin_active('meta-box/meta-box.php')) {
		trait RWMB_Social_Field_Trait {
			public static function begin_html( array $field) : string {
				$field_label = '';
				if ( $field['name'] ) {
					$field_label = sprintf(
