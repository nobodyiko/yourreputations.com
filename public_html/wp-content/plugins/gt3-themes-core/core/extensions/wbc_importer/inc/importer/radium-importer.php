...
// Don't duplicate me!
if(!class_exists('GT3_Radium_Theme_Importer')) {

	class GT3_Radium_Theme_Importer {
		...
		public function check_previous_meta($continue, $post_id, $meta_key, $meta_value, $unique){
			...
			if(count($old_value) == 1) {
				if($old_value[0] === $meta_value) {
					return false;
				} else if($old_value[0] !== $meta_value) {
		...
		public function import_widgets($data){
			...
			foreach($data as $sidebar_id => $widgets) {
				...
				foreach($widgets as $widget_instance_id => $widget) {
					...
					$widget = apply_filters('radium_theme_import_widget_settings', $widget, $id_base);
