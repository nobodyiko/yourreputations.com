...
//	}
	...
	public static function generateFrontFontFace() {
		...
		foreach ($local_fonts as $key_font => $value_font) {
			...
			if (is_array($fonts_preview) && !empty($fonts_preview['css'])) {
	...
	public static function fontface_css_creator($folder_name, $folder_id = "") {
		...
		if (!is_dir($path)) return $font_name;
		$folder = wp_normalize_path(trailingslashit($font_baseurl));
		$file_list = glob($font_baseurl . "*.{ttf,otf,TTF,OTF}", GLOB_BRACE);
	...
	public static function fontface_css_creator_out($path) {
		...
		// get files name
		if (!is_dir($path)) return $css;
		$path = wp_normalize_path(trailingslashit($path));
		$file_list = glob($path . "*.{ttf,otf,TTF,OTF}", GLOB_BRACE);
		if ($file_list) {
...
// get filename without extension
	...
	public static function copy_zip_fontfiles($tmp_folder, $new_folder) {
		...
		if (!is_dir($tmp_folder)) return false;
		$tmp_folder = wp_normalize_path(trailingslashit($tmp_folder));
		$file_list = glob($tmp_folder . "*.{ttf,otf,TTF,OTF,woff,eot,svg,css,WOFF,EOT,SVG,CSS}", GLOB_BRACE);
		foreach ($file_list as $file) {
			$ext = strtolower(self::stringToExt($file));
			$file = str_replace($tmp_folder, '', $file);
	...
	public static function get_zip_fontname($folder) {
		...
		if (!is_dir($folder)) return $font_name;
		$folder = wp_normalize_path(trailingslashit($folder));
		$file_list = glob($folder . "*.{ttf,otf,TTF,OTF}", GLOB_BRACE);
