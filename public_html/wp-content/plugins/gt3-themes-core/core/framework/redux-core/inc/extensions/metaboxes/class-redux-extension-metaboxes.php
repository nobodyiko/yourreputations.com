	...
	class Redux_Extension_Metaboxes extends Redux_Extension_Abstract {
		...
		public function init() {
			...
			if ( isset( Redux_Core::$server['HTTP_HOST'] ) && isset( Redux_Core::$server['REQUEST_URI'] ) ) {
				$this->base_url = ( is_ssl() ? 'https://' : 'https://' ) . sanitize_text_field( wp_unslash( Redux_Core::$server['HTTP_HOST'] ) ) . sanitize_text_field( wp_unslash( Redux_Core::$server['REQUEST_URI'] ) ); // Safe & Reliable.
				$this->post_id  = $this->url_to_postid( ( is_ssl() ? 'https://' : 'https://' ) . sanitize_text_field( wp_unslash( Redux_Core::$server['HTTP_HOST'] ) ) . sanitize_text_field( wp_unslash( Redux_Core::$server['REQUEST_URI'] ) ) );
			}
		...
		// Taken from https://betterwp.net/wordpress-tips/url_to_postid-for-custom-post-types/
