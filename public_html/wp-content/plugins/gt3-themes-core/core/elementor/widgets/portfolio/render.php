...
if($query->have_posts()) {
	...
	$settings['posts_per_page']   = $query_args['posts_per_page'];
	...
	$data_wrapper                 = array(
		...
		'filter_ajax'    => ((bool) ($settings['filter_ajax'])),
		'lazyload'       => $settings['lazyload'],
		'posts_per_page' => intval($query_args['posts_per_page']),
		'firstLoad'      => intval($settings['posts_per_page']),
		'load_items'     => intval($settings['load_items']),
		...
		'settings'       => array(
			'filter_ajax'  => $settings['filter_ajax'],
		...
		if((bool) ($settings['use_filter'])) {
			...
			<div class="isotope-filter<?php echo !empty($settings['filter_style']) ? ' isotope-filter--'.$settings['filter_style'] : ''; ?>">
				...
				if(count($query_raw['taxonomy']) > 1) {
					$count_all = $widget->count_terms($query_raw['taxonomy']) ;
					
					foreach($widget->get_taxonomy($query_raw['taxonomy']) as $cat_slug) {
						$count = $widget->count_terms($cat_slug['slug']);
						...
						if($settings['filter_style'] == 'links') {
							...
							echo '<a href="'.$url.'" data-count="'.$count.'" data-filter=".'.esc_attr($cat_slug['slug']).'" '.($_REQUEST[$portfolio_query_arg] == $cat_slug['slug'] ? ' class="active"' : '').'>'.esc_html($cat_slug['name']).'</a>';
						} else {
							echo '<a href="#" data-count="'.$count.'" data-filter=".'.esc_attr($cat_slug['slug']).'">'.esc_html($cat_slug['name']).'</a>';
						}
					}
					...
					$count_all = $widget->count_terms($terms) ;
					foreach($cats as $cat) {
						$count = $widget->count_terms($cat->slug);
						
						$permalink = get_permalink();
						$permalink = add_query_arg($portfolio_query_arg, $cat->slug, $permalink);
						echo '<a href="'.esc_url($permalink).'"  data-count="'.$count.'" data-filter=".'.esc_attr($cat->slug).'"'.($_REQUEST[$portfolio_query_arg] == $cat->slug ? ' class="active"' : '').'>';
				...
				echo ($settings['filter_style'] == 'links'
						? '<a href="'.get_permalink().'" data-count="'.$count_all.'" data-filter="all" '.($_REQUEST[$portfolio_query_arg] == '' ? ' class="active"' : '').'>'.esc_html($settings['all_title']).'</a>'
						: '<a href="#"  data-count="'.$count_all.'" class="active" data-filter="all">'.esc_html($settings['all_title']).'</a>').$content;
				
				//				if($settings['filter_style'] == 'links') {
				//					echo '<a href="'.get_permalink().'" data-filter="all" '.($_REQUEST[$portfolio_query_arg] == '' ? ' class="active"' : '').'>'.esc_html($settings['all_title']).'</a>';
				//				} else {
				//					echo '<a href="#" class="active" data-filter="all">'.esc_html($settings['all_title']).'</a>';
				//				}
