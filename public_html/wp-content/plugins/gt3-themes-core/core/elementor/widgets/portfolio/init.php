...
if(!class_exists('ElementorModal\Widgets\GT3_Core_Elementor_Widget_Portfolio')) {
	class GT3_Core_Elementor_Widget_Portfolio extends \ElementorModal\Widgets\GT3_Core_Widget_Base {
		...
		public function ajax_handler(){
			...
			if(isset($_POST['with_exclude']) && ($_POST['with_exclude'] === true || $_POST['with_exclude'] == 'true')) {
				$_POST['with_exclude'] = true;
			} else {
				$_POST['with_exclude'] = false;
			}
			...
			$_POST['settings']['ajax'] = true;

			while($query->have_posts()) {
				$query->the_post();
				$respond .= $this->renderItem($_POST['use_filter'], $_POST['show_title'], $_POST['show_category'], $render_index, $_POST['settings']);
				if($_POST['random'] || $_POST['with_exclude']) {
					$_POST['query']['exclude'][] = intval($query->post->ID);
				}
				$render_index++;
				$rendered++;
			}
			$_POST['query']['exclude'] = array_map('intval', $_POST['query']['exclude']);
			...
			wp_json_encode(
				array(
					...
					'hasMore'    => $rendered < $query->found_posts,
				)
			)
			);
		}
		...
		public function renderItem($use_filter = false, $show_title = false, $show_category = false, $render_index = 0, $settings = false){
			...
			if(!key_exists('filter_name', $settings) && key_exists('filter_ajax', $settings) && $settings['filter_ajax']) {
				$item_class .= ' first-load';
				//				$settings['filter_name'] = 'all';
			}
			if(key_exists('filter_name', $settings) && key_exists('filter_ajax', $settings) && $settings['filter_ajax']) {
				$item_class .= ' ajax_loaded-'.$settings['filter_name'];
			}
			...
			$render .= '<a href="'.esc_url($linkToTheWork).'" class="lightbox" title="'.esc_attr(get_the_title()).'" '.$target.'>';
			...
			if(((bool) $show_title || (bool) $show_category || (bool) $show_description) && (!empty($title) || !empty($item_category) || !empty($post_descr))) {
				...
				if((bool) $show_title && !empty($title)) {
					$render .= '<'. apply_filters('gt3_portfolio_packery_title_tag', 'h4') .' class="title">'.get_the_title().'</'. apply_filters('gt3_portfolio_packery_title_tag', 'h4') .'>';
				}
		...
		function count_terms($terms){
			...
			foreach($terms as $term) {
				...
				$count  = array_merge($count, $_posts->posts);
			}
