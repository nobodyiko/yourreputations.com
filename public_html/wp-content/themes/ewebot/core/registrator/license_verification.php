...
add_filter('gt3/core/registration/get_purchase_url', function() {
	return 'https://gt3themes.com/wordpress/ewebot-seo-marketing-agency-wordpress-theme/';
});
...
add_filter(
	...
	function() {
		return array(
			'revslider',
			'gt3-elementor-unlimited-charts',
			'gt3-themes-core',
			'wpda-builder-pro'
		);
	}
...
if (class_exists('GT3\ThemesCore\Registration')) {
	return;
}
...
function gt3_registration($code = '',$action_type = 'register'){
	...
	$gt3_is_child    = $my_theme->get('Template');
	if(!empty($gt3_is_child)) {
		$my_theme = wp_get_theme($gt3_is_child);
	}
...
function gt3_check_theme_update ( $transient ){
    ...
    $plugins = array('revslider','gt3-elementor-unlimited-charts','gt3-themes-core','wpda-builder-pro');
	$my_theme = wp_get_theme();
	$gt3_is_child    = $my_theme->get('Template');
	if(!empty($gt3_is_child)) {
		$my_theme = wp_get_theme($gt3_is_child);
