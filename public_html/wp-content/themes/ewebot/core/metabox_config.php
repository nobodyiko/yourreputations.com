...
function gt3_page_layout_meta_boxes( $meta_boxes ) {
	...
	global $pagenow;
	if (key_exists('post', $_REQUEST) && class_exists( 'WooCommerce' ) && ($pagenow === 'post.php') && (gt3_option('modern_shop') == '1' || true === gt3_option('modern_shop')) && in_array((int)($_REQUEST['post']), array(wc_get_page_id( 'cart' ), wc_get_page_id( 'checkout' )))) {
		return $meta_boxes;
	}
